from until import return_queue_index, state2index_2
import numpy as np
import copy
class TSRA_AGENT(object):
    def __init__(self, D, arrival_rate, learning_rate, gamma, length):
        ...
        self.length = length
        # self.Q_table = np.zeros(shape=((self.length+2) * 4, 2), dtype=float)
        # self.policy = np.zeros(shape=((self.length+2) * 4, 1), dtype=float)
        self.Q_table = np.zeros(shape=(3 * 4, 2), dtype=float)
        self.policy = np.zeros(shape=(3 * 4, 1), dtype=float)
        self.rho = 0
    ...
    def update_Q_table(self, observation):
        # index_1 = state2index(L2=self.state[-3], O=self.state[-4])
        # index_2 = state2index(L2=self.state[-1], O=self.state[-2])
